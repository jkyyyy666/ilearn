...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (isGithubConfigured()) {
      downloadBackup().then(r => {
        if (r.success) {
          setStreak(calculateStreak());
          addToast("已从 GitHub 恢复备份数据", "success");
        }
        // 如果是首次使用没有备份文件，不显示错误
      });
    }
  ...
  return (
    <>
      <StarBackground />
      <div style={{
        minHeight: "100vh", position: "relative", background: "transparent",
        ...
        <div style={{ textAlign: "center", marginBottom: 48 }}>
          ...
          <p style={{ fontSize: 16, color: "#475569", maxWidth: 300, margin: "0 auto", lineHeight: 1.6 }}>
            --天行健，君子以自强不息
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, width: "100%", maxWidth: 360 }}>
          <button
            ...
            style={{
              padding: "28px 24px", cursor: "pointer", border: "2px solid rgba(255,255,255,0.1)",
              textAlign: "left", display: "flex", alignItems: "center", gap: 20, transition: "all 0.2s",
            }}
        ...
        <div className="card" style={{ marginTop: 32, padding: "16px 20px", width: "100%", maxWidth: 360, textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
            {[5, 10, 15, 20, 30].map((n) => (
              <button
                key={n}
                className={"btn btn-sm " + (dailyGoal === n ? "btn-primary" : "btn-secondary")}
                onClick={() => {
                  setDailyGoal(n);
        ...
        <div style={{ marginTop: 40, fontSize: 12, color: "rgba(255,255,255,0.4)", textAlign: "center" }}>
